...
在 VS Code 中浏览、搜索并用 DuckDB SQL 查询工作区内的数据文件。侧边栏列出支持的格式，点击后在原生 SQL 编辑器中编辑查询，结果在 React 表格 Webview 中分页展示。
...
- **虚拟 SQL 文档**：`dataviewer-sql:` 方案，短文件名（如 `people.csv.sql`），可编辑
- **SQL 补全**：DuckDB 关键字、函数及列名
- **运行方式**：每条语句的 **Run Statement** CodeLens，或 **Ctrl+Enter** / **Cmd+Enter** 运行整篇文档
...
3. **打开数据文件**：点击列表中的文件；XLSX 先展开再选择 Sheet。
4. **编辑 SQL**：打开后会生成默认查询（如 `SELECT * FROM read_csv_auto('...')`），可在虚拟 SQL 标签页中修改。
5. **执行查询**：
   - 点击语句上方的 **Run Statement** CodeLens，或
   - 在 SQL 编辑器中按 **Ctrl+Enter**（macOS：**Cmd+Enter**）运行全文
6. **查看结果**：表格 Webview 支持分页（默认每页 500 行）、排序与列过滤。
...
| 命令 | 说明 |
|------|------|
| `Data Viewer: Refresh` | 重新扫描工作区数据文件 |
| `Data Viewer: Open Data File` | 打开指定数据目标（由侧边栏调用） |
| `Data Viewer: Run SQL Query` | 运行当前 SQL 文档全文 |
| `Data Viewer: Run SQL Statement` | 运行单条 SQL（CodeLens） |
...
src/                    # 扩展主进程（TypeScript）
  ...
  viewer/               # 数据查看器、SQL 文档、CodeLens、补全
  utils/                # 路径、XLSX、SQL 语句解析
webview/                # 数据表格 React Webview
...
- **Webview UI**：React 19、TanStack Table 8
- **构建**：Vite 8
